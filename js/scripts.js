...
jQuery(document).ready(function($) {
  ...
  $('#menu').click(function(){
    $('#burger, #full-screen-nav').toggleClass('open');
  ...
  $(".scroll-to-link").click(function(e) {
    ...
    page.animate({ scrollTop: pos }, 1000, function(){
  ...
  if ($("body").hasClass("home")) {
	  var countBrandsOptions = {
	    useEasing: true, 
	    useGrouping: true, 
	    separator: ',', 
	    decimal: '.', 
	  };
	  var countBrands = new CountUp('countBrands', 0, 3, 0, 3.5, countBrandsOptions);

	  var countProductsOptions = {
	    useEasing: true, 
	    useGrouping: true, 
	    separator: ',', 
	    decimal: '.'
	  };
	  var countProducts = new CountUp('countProducts', 0, 250, 0, 3.5, countProductsOptions);

	  var ranges = [
	    { divider: 990000, suffix: 'MM' },
	    { divider: 1000, suffix: 'K' }
	  ];

	  var countUnitsOptions = {
	    useEasing: true, 
	    useGrouping: true, 
	    separator: ',', 
	    decimal: '.', 
	  ...
	  var countUnits = new CountUp('countUnits', 10000, 3000000, 0, 3.5, countUnitsOptions);

	  var quotes = $(".press-quote");
	  var quoteIndex = -1;
	  function pressQuotes() {
	    ...
	    quotes.eq(quoteIndex % quotes.length)
	        .fadeIn(1000)
	        .delay(1000)
	        .fadeOut(1000, pressQuotes);
	  }

    $("#how-we-work-timeline").owlCarousel({
      items: 1,
      dots: true,
      loop: true,
      dotsData: true,
    ...
    $(window).scroll(function(){
      if ($('#site-intro').visible(true)) {
        $('#site-intro').addClass('animated fadeInUp');
      } else {
        
      }
      if ($('#count-up-container').visible(true)) {
        setTimeout(function() {
          if (!countBrands.error) {
            countBrands.start();
          } else {
            console.error(countBrands.error);
          }
          if (!countProducts.error) {
            countProducts.start();
          } else {
            console.error(countProducts.error);
          }
          if (!countUnits.error) {
            countUnits.start();
          } else {
            console.error(countUnits.error);
          }
        }, 750)
      } else {
  ...
  $('#team-toggle').click(function(){
    $('#team-grid').slideToggle('medium');
    //$('#team-arrow').toggleClass('flipped');
  });
  $('#team-waving-hand').hover(function(e){
  ...
  if ($("body").hasClass("careers")) {
  	if (document.location.hash) {
    	$.featherlight(document.location.hash);                   
	}
    var $jobs = $("#careers-grid > .job");
    var $companies = $("#filter-company").change(function(e) {
      var activeCo = $(this).find("option:selected").val();
      var currentDept = $("#filter-dept").find("option:selected").val();
      $jobs
        .hide()
        .filter( "." + activeCo )
        .filter( "." + currentDept )
      ...
      //window.location.hash = activeCo;
      e.preventDefault();

    });
    var $departments = $("#filter-dept").change(function(e) {
      var activeDept = $(this).find("option:selected").val();
      var currentCo = $("#filter-company").find("option:selected").val();
      $jobs
        .hide()
        .filter( "." + activeDept )
        .filter( "." + currentCo )
    ...
    $(document).on('change', '.career-filter', function(){
      if($('#careers-grid').children(':visible').length == 0) {
        $('#no-jobs').fadeIn(450);
      } else {
        $('#no-jobs').hide();
      }
